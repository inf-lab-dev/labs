...
```bash-vue
wget -O - {{ $url('material/lab-weather.zip.sh') }} | bash
```
...
In dieser Teilaufgabe kombinieren wir die Filterlogik aus [Teilaufgabe 5](#teilaufgabe-5) und wenden die implementierten Filter auf die Liste `cities_weather` an. Ziel ist es, die Liste so zu filtern, dass nur die Städte übrig bleiben, die den angegebenen Kriterien entsprechen. Diese Städte werden anschließend auf der Konsole ausgegeben. Die gesamte Logik wird in der `main`-Funktion eingebettet.

Zunächst solltest du die Filter anlegen, indem du eine Instanzen der Klassen `TempRangeFilter` mit beliebigen Kriterien erstellst. Zum Beispiel könnten die Kriterien so aussehen: Ein _Temperaturbereich von 5°C bis 25°C_.

Anschließend sollte über die Liste `cities_weather` iteriert werden, die die Wetterdaten der Städte enthält. Für jede Stadt wird überprüft, ob das definierte Filterkriterium erfüllt ist. Wenn eine Stadt den Filter erfüllt, soll `<city_name> matches all filter criteria!` ausgeben werden. `<city_name>` soll entsprechend durch den Namen der Stadt ersetzt werden. Erfüllt eine Stadt nicht alle Kriterien, soll `<city_name> does not match all filter criteria!` ausgeben werden.
